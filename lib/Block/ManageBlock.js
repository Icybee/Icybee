!function (Brickrouge) {
	...
	Brickrouge.register('ManageBlock', (element, options) => new ManageBlock(element, options))
...
!function() {
	...
	{
		const actionbarControls = document.body.getElement('.actionbar-controls')
		const currentControls = document.body.getElement('.actionbar .listview-controls')
		const controls = fragment.getElement('.listview .listview-controls')
		const startControl = controls.getElement('[name="start"]')
		const limitControl = controls.getElement('[name="limit"]')
	...
	window.addEvent('domready', function() {
		...
		const container = document.body.getElement('.listview-search')
		const search = container.getElement('input')
		let searchLast = null
	...
	const OperationHandler = new Class({
		...
		run: function()
		{
			new Request.Element({
				...
				{
					const managerEl = manager.element
	...
	const ActionBarOperations = new Class({
		...
		{
			const pattern = this.options[count === 1 ? 'patternOne' : 'patternOther' ]
		...
		{
			const handler = new OperationHandler(operation, this.selected)
			const el = this.element;
	...
	Brickrouge.register('ActionBarOperations', (element, options) => new ActionBarOperations(element, options))
