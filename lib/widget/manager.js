		...
		if (checkboxes.length)
		{
			...
			(
				'click', function(ev)
				{
					...
					if (target.getParent('.btn-group') || target.hasClass('btn-group'))
					{
						return;
					}
	...
	queryOperation: function(operation, keys)
	{
		...
		({
			...
			onSuccess: function(response)
			{
				...
				html += '<button name="ok" class="btn-warning">' + rc.confirm[1] + '</button>';
				html += '</div>';
		...
		var iterator = function()
		{
			...
			({
				...
				onSuccess: function(response)
				{
					...
					if (response.success)
					{
						message.set('html', response.success);
					}
