...
({
	...
	initialize: function()
	{
		...
		searchForm.getElement('button').addEvent
		(
			'click', function(ev)
			{
				ev.key = 'esc';
		...
		actionbar.addEvent('click', function(ev) {

			var target = ev.target

			if (target.tagName != 'A' || !target.getParent('.browse')) return

			ev.stop()

			this.getBlock(target.get('href').substring(1))
	...
	attach: function(el)
	{
		...
		this.destination = this.element[ICanBoogie.Operation.DESTINATION].value;
		...
		var checkboxes_master = el.getElement('thead .key input[type=checkbox]');
		...
		if (checkboxes.length)
		{
			...
			if (jobs)
			{
				jobs.set('tween', { duration: 'short', link: 'cancel' });
			}
			...
			(
				'click', function(ev)
				{
					...
					if (target.getParent('.btn-group') || target.hasClass('btn-group') || target.getParent('trigger') || target.hasClass('trigger'))
					{
						return;
					}
		...
		var actionBarBrowse = document.body.getElement('.actionbar .browse')
		...
		if (browse)
		{
			if (!actionBarBrowse)
			{
				actionBarBrowse = browse.clone()
				actionBarBrowse.inject(document.body.getElement('.navbar-search'), 'before')
			}
			else
			{
				actionBarBrowse.getElement('a.previous').set('href', browse.getElement('a.previous').get('href'))
				actionBarBrowse.getElement('a.next').set('href', browse.getElement('a.next').get('href'))
			}
		}
		else if (actionBarBrowse)
		{
			actionBarBrowse.destroy()
		}
	...
	startOperation: function(operation, params)
	{
		...
		var iterator = function()
		{
			...
			var op = new Request.API
			({
				url: this.destination + '/' + key + '/' + operation,
				...
				onFailure: function(response)
				{
					...
					this.finishOperation(operation, response.errors);
				},
